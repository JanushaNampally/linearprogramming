...
from .forms import LPForm  # type: ignore
...
def simplex_method(request):
    form = LPForm(request.POST or None)
    solution = None
    if request.method  == 'POST':
        if form.is_vaild():
            equation = form.cleaned_data['equation']
            solution = "Stepss to solve simplex methods" 
            
            
    return render(request, 'simplex.html', {'form': form, 'solution': solution})
